      ...
      <main className="max-w-7xl mx-auto px-4 py-6">
        ...
        <div className="grid grid-cols-2 gap-px mb-px" style={{ background: '#1c3849' }}>
          ...
          <div style={{ background: '#001520', padding: '1rem' }}>
            <div style={{ fontSize: '9px', color: '#ffffff', textTransform: 'uppercase', letterSpacing: '0.2em', marginBottom: '0.75rem' }}>Latest from GOV.UK</div>
            {leadStory && (
              <div style={{ marginBottom: '1rem', paddingBottom: '1rem', borderBottom: '1px solid #1c3849' }}>
                ...
                <div style={{ fontSize: '11px', color: '#ffffff' }}>{leadStory.organisations?.[0]?.title || 'GOV.UK'} · {new Date(leadStory.public_timestamp).toLocaleDateString('en-GB', { day: 'numeric', month: 'short' })}</div>
              </div>
            )}
            {otherNews.map((item: any, i: number) => (
              <div key={i} style={{ paddingBottom: '8px', marginBottom: '8px', borderBottom: '0.5px solid #1c3849' }}>
                <div style={{ fontSize: '12px', color: '#ffffff', lineHeight: 1.3 }}>{item.title}</div>
        ...
        <div style={{ background: '#001520', padding: '1rem', borderTop: '1px solid #1c3849', marginBottom: '1px' }}>
          <div style={{ fontSize: '9px', color: '#ffffff', textTransform: 'uppercase', letterSpacing: '0.2em', marginBottom: '0.75rem' }}>Live Activity</div>
          <div>
            {activity?.map((item, i) => (
              <Link href="/transparency/ministers-hospitality" key={i} style={{ display: 'flex', gap: '10px', padding: '6px 0', borderBottom: '0.5px solid #1c3849', textDecoration: 'none', alignItems: 'flex-start' }}>
                ...
                <div>
                  <div style={{ fontSize: '12px', color: '#ffffff', lineHeight: 1.4 }}>{item.minister_name} — {item.donor} · £{Number(item.amount).toLocaleString()}</div>
        ...
        <div className="grid grid-cols-2 gap-px mb-px" style={{ background: '#1c3849' }}>
          <div style={{ background: '#001520', padding: '1rem' }}>
            <div style={{ fontSize: '9px', color: '#ffffff', textTransform: 'uppercase', letterSpacing: '0.2em', marginBottom: '0.75rem' }}>Latest Contracts</div>
            {contracts?.map((c, i) => (
          ...
          <div style={{ background: '#001520', padding: '1rem' }}>
            <div style={{ fontSize: '9px', color: '#ffffff', textTransform: 'uppercase', letterSpacing: '0.2em', marginBottom: '0.75rem' }}>Latest Donations</div>
            {donations?.map((d, i) => (
        ...
        <div className="grid grid-cols-4 gap-px" style={{ background: '#1c3849' }}>
          {[
            ...
            <div key={i} style={{ background: '#001520', padding: '0.75rem' }}>
              ...
              <div style={{ fontSize: '9px', color: '#ffffff', textTransform: 'uppercase', letterSpacing: '0.1em' }}>{s.label}</div>
            </div>
          ))}
        </div>
        ...
        <div style={{ marginTop: '1rem', paddingTop: '1rem', borderTop: '1px solid #1c3849', display: 'flex', gap: '2rem', flexWrap: 'wrap' }}>
          {[
            ...
            <Link key={link.href} href={link.href} style={{ fontSize: '12px', color: '#ffffff', textDecoration: 'none' }}>{link.label} →</Link>
          ))}
        </div>
