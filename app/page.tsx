    ...
    <div style={{ minHeight: '100vh', background: '#1a1a1a', color: '#fff' }}>
      ...
      <main style={{ maxWidth: '1280px', margin: '0 auto', padding: '0 1.5rem 3rem' }}>
        ...
        <div style={{ padding: '1.25rem 0', borderBottom: '3px solid #fff', display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: '0' }}>
          <div style={{ fontSize: '11px', color: '#999999', letterSpacing: '0.2em', textTransform: 'uppercase' }}>{today}</div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '1px', background: '#333', borderBottom: '1px solid #333' }}>
          ...
          <div style={{ background: '#1a1a1a', padding: '1.5rem 2rem' }}>
            {leadStory && (
              <>
                ...
                <div style={{ fontSize: '11px', color: '#999999', marginBottom: '1.5rem' }}>{leadStory.organisation} · {leadStory.published_at ? new Date(leadStory.published_at).toLocaleDateString('en-GB', { day: 'numeric', month: 'short' }) : ''}</div>
              </>
            )}
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
              {otherStories.map((story, i) => (
                <div key={i} style={{ borderLeft: '2px solid #333', paddingLeft: '0.75rem' }}>
                  <div style={{ fontSize: '9px', color: '#999999', textTransform: 'uppercase', letterSpacing: '0.15em', marginBottom: '3px' }}>{story.organisation}</div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
              <div>
                <div style={{ fontSize: '9px', color: '#999999', textTransform: 'uppercase', letterSpacing: '0.2em', borderBottom: '1px solid #333', paddingBottom: '6px', marginBottom: '8px' }}>Latest Contracts</div>
                {contracts?.map((c, i) => (
                  <Link href="/transparency/contracts" key={i} style={{ display: 'block', marginBottom: '10px', textDecoration: 'none', borderLeft: '2px solid #333', paddingLeft: '0.75rem' }}>
                    ...
                    <div style={{ fontSize: '11px', color: '#999999', marginTop: '2px' }}>{c.supplier} · £{c.value ? Number(c.value).toLocaleString() : 'undisclosed'}</div>
                  </Link>
                ))}
              </div>
              <div>
                <div style={{ fontSize: '9px', color: '#999999', textTransform: 'uppercase', letterSpacing: '0.2em', borderBottom: '1px solid #333', paddingBottom: '6px', marginBottom: '8px' }}>Latest Donations</div>
                {donations?.map((d, i) => (
                  <Link href="/transparency/donations" key={i} style={{ display: 'block', marginBottom: '10px', textDecoration: 'none', borderLeft: '2px solid #333', paddingLeft: '0.75rem' }}>
                    ...
                    <div style={{ fontSize: '11px', color: '#999999', marginTop: '2px' }}>{d.recipient_name} · £{Number(d.amount).toLocaleString()}</div>
                  </Link>
                ))}
              </div>
            </div>
          ...
          <div style={{ background: '#111', padding: '1.5rem', display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
            ...
            {/* Bills voting */}
            <div>
              <div style={{ fontSize: '9px', color: '#999999', textTransform: 'uppercase', letterSpacing: '0.2em', borderBottom: '1px solid #222', paddingBottom: '6px', marginBottom: '1rem' }}>Public vs Parliament</div>
              {bills?.map((bill) => {
                ...
                return (
                  <Link href={`/bills/${bill.id}`} key={bill.id} style={{ display: 'block', marginBottom: '1rem', textDecoration: 'none' }}>
                    ...
                    <div style={{ fontSize: '11px', color: '#999999' }}>{yesPct}% support · {total.toLocaleString()} votes</div>
                  </Link>
                )
              })}
            </div>
            ...
            {/* Revolving door */}
            <div>
              <div style={{ fontSize: '9px', color: '#999999', textTransform: 'uppercase', letterSpacing: '0.2em', borderBottom: '1px solid #222', paddingBottom: '6px', marginBottom: '1rem' }}>Revolving Door</div>
              {revolving?.map((r, i) => (
                <Link href="/transparency/revolving-door" key={i} style={{ display: 'block', marginBottom: '10px', textDecoration: 'none' }}>
                  ...
                  <div style={{ fontSize: '11px', color: '#999999', marginTop: '2px' }}>{r.previous_role}</div>
                </Link>
              ))}
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', marginTop: 'auto' }}>
              {[
                ...
                <div key={i} style={{ border: '1px solid #222', padding: '8px', textAlign: 'center' }}>
                  ...
                  <div style={{ fontSize: '9px', color: '#999999', textTransform: 'uppercase', letterSpacing: '0.1em' }}>{s.label}</div>
                </div>
              ))}
            </div>
