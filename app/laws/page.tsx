  ...
  return (
    <div className="min-h-screen bg-[#0a0f1a]">
      ...
      <main className="max-w-7xl mx-auto px-6 py-6">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-4xl font-bold text-white mb-2">Laws</h1>
        ...
        {!laws || laws.length === 0 ? (
          <div className="text-center py-12">
            <p className="text-gray-400">No laws found with Royal Assent status</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {laws.map((law) => (
              <Link
                key={law.id}
                href={`/bills/${law.id}`}
                className="bg-white/5 backdrop-blur-sm rounded-lg border border-white/10 hover:border-white/20 transition-all p-6"
              >
                {/* Law Title */}
