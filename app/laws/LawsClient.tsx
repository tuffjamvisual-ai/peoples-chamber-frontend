    ...
    <main className="max-w-7xl mx-auto px-4 sm:px-6 py-4 sm:py-6">
      ...
      <div className="mb-6">
        <input
          ...
          className="w-full px-4 py-3 bg-[#2a2a2a] text-white rounded-lg border border-[#2a2a2a] focus:border-[#ffffff] focus:outline-none"
        />
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
          {filteredLaws.map((law) => (
            <Link
              key={law.id}
              href={`/bills/${law.id}`}
              className="bg-[#111111] border border-[#2a2a2a] rounded-lg p-4 sm:p-6 hover:border-[#ffffff] transition-colors flex flex-col"
            >
              <div className="flex items-center gap-2 mb-3">
                ...
                {law.originating_house && (
                  <span className="text-xs text-[#999999]">{law.originating_house}</span>
                )}
              </div>
              ...
              {law.plain_summary && (
                <p className="text-[#999999] text-xs mb-4 line-clamp-3 flex-1">
                  {law.plain_summary}
                </p>
              )}

              <div className="flex items-center justify-between text-xs text-[#999999] mt-auto">
                {law.sponsor_name && (
