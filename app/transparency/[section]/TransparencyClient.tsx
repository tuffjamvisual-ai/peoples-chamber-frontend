...
function Pagination({
  ...
  return (
    <div className="flex items-center justify-between mt-8 pt-6 border-t border-[#5a5a5a]">
      <button
        onClick={onPrev}
        disabled={page === 0}
        className="px-4 py-2 text-[14px] uppercase tracking-[0.2em] font-mono border border-[#5a5a5a] text-white hover:border-[#ffffff] hover:text-white transition-colors disabled:opacity-30 disabled:cursor-not-allowed"
      >
        ← Previous
      </button>
...
export default function TransparencyClient({ rows, sectionTitle, section, total, searchQuery }: Props) {
  ...
  if (rows.length === 0 && !searchQuery) {
    return (
      <p className="text-white text-[13px] leading-[1.7] border-t border-[#5a5a5a] pt-8">
  ...
  return (
    <>
        ...
        {isDonations ? (
          <form method="GET" className="flex gap-2 max-w-md">
            <input
              ...
              className="flex-1 bg-[#404040] border border-[#5a5a5a] text-white text-[13px] rounded-sm px-4 py-3 leading-[1.7] placeholder:text-white focus:outline-none focus:border-[#ffffff] transition-colors"
            />
            <button
              type="submit"
              className="px-4 py-3 text-[14px] uppercase tracking-[0.2em] font-mono border border-[#5a5a5a] text-white hover:border-[#ffffff] hover:text-white transition-colors whitespace-nowrap"
            >
              Search
            </button>
          </form>
          ...
          <>
            <input
              ...
              className="w-full max-w-md bg-[#404040] border border-[#5a5a5a] text-white text-[13px] rounded-sm px-4 py-3 leading-[1.7] placeholder:text-white focus:outline-none focus:border-[#ffffff] transition-colors"
            />
            {query && (
      ...
      {rows.length === 0 && searchQuery ? (
        ...
        <>
          <ul className="space-y-px bg-[#404040] border border-[#5a5a5a]">
            {paged.map((row, i) => {
              ...
              return (
                <li key={i} className="bg-[#404040] p-5 border-l-2 border-l-[#ffffff]">
                  ...
                  {previousRole && (
                    <p className="text-white text-[13px] leading-[1.7] mb-1">{previousRole}</p>
                  )}
          ...
          {rows.map((row, i) => {
            ...
            return (
              <li key={i} className="bg-[#404040] p-5 border-l-2 border-l-[#ffffff]">
                ...
                <p className="text-[13px] text-white leading-[1.7] mb-1">
        ...
        <>
          <ul className="space-y-px bg-[#404040] border border-[#5a5a5a]">
            {paged.map((row, i) => {
              ...
              return (
                <li key={i} className="bg-[#404040] p-5 border-l-2 border-l-transparent hover:border-l-[#ffffff] transition-colors">
