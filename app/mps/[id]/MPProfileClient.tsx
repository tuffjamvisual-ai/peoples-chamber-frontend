...
export default function MPProfileClient({
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-4 gap-px bg-[#2a2a2a] border border-[#2a2a2a]">
      {/* Sidebar */}
      <aside className="lg:col-span-1 bg-[#1a1a1a]">
        <div className="lg:sticky lg:top-16">
          <p className="text-[10px] uppercase tracking-[0.25em] text-[#999999] font-medium px-4 pt-5 pb-3">
            Sections
          </p>
          <nav>
            {menuItems.map((item) => {
              ...
              return (
                <button
                  ...
                  className={
                    ...
                    (active
                      ? 'text-white bg-[#111827] border-l-[#ffffff] font-semibold'
                      : 'text-[#999999] border-l-transparent hover:text-white hover:bg-[#111827]')
                  }
                >
                  {item.label}
                </button>
      ...
      {/* Main */}
      <div className="lg:col-span-3 bg-[#1a1a1a] p-6 sm:p-8">
              ...
              <div className="space-y-px bg-[#2a2a2a] border border-[#2a2a2a]">
                <Card title="Parliamentary office" partyColour={partyColour}>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-[13px] leading-[1.7]">
                    <div className="text-[#999999] space-y-0.5">
                    ...
                    <div className="space-y-1">
                      {contact.phone && (
                        <p className="text-[#999999]">
                          <span className="text-[10px] uppercase tracking-[0.25em] text-[#999999] mr-2">Phone</span>
                          <span className="text-white font-mono text-[13px]">{contact.phone}</span>
                        </p>
                      )}
                      {contact.email && (
                        <p className="text-[#999999]">
                          <span className="text-[10px] uppercase tracking-[0.25em] text-[#999999] mr-2 block sm:inline">Email</span>
              ...
              <div className="space-y-px bg-[#2a2a2a] border border-[#2a2a2a]">
                {representations.map((rep: any, idx: number) => (
                  <Card key={idx} title={rep.name} partyColour={partyColour}>
                    <p className="text-[12px] text-[#999999] font-mono leading-[1.7]">
                    ...
                    {rep.additionalInfo && <p className="text-[13px] text-[#999999] mt-2 leading-[1.7]">{rep.additionalInfo}</p>}
                  </Card>
                ))}
              </div>
            ) : (
        ...
        {activeSection === 'voting' && (
          <Section title="Voting Record">
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-px bg-[#2a2a2a] border border-[#2a2a2a] mb-8">
            ...
            {votes && votes.length > 0 ? (
              <ul className="space-y-px bg-[#2a2a2a] border border-[#2a2a2a]">
                {votes.slice(0, 20).map((vote: any) => (
                  <li
                    key={vote.id}
                    className="flex items-start justify-between gap-4 bg-[#1a1a1a] p-4 border-l-2"
            ...
            {sponsoredBills && sponsoredBills.length > 0 ? (
              <ul className="space-y-px bg-[#2a2a2a] border border-[#2a2a2a]">
                {sponsoredBills.map((bill) => (
                  <li key={bill.id} className="bg-[#1a1a1a]">
                    <Link
                      href={'/bills/' + bill.id}
                    ...
                    >
                      ...
                      <div className="flex items-center gap-3 text-[11px]">
                        <span className="text-[#999999] font-mono uppercase tracking-[0.15em]">{bill.current_stage || 'Unknown'}</span>
              ...
              <div className="space-y-10">
                {Object.keys(interestsByCategory).map((categoryName) => (
                  <div key={categoryName}>
                    <h3 className="text-[10px] uppercase tracking-[0.25em] mb-4 pb-3 border-b border-[#2a2a2a] font-semibold" style={{ color: ACCENT }}>
                      {categoryName}
                    </h3>
                    <ul className="space-y-px bg-[#2a2a2a] border border-[#2a2a2a]">
                      {interestsByCategory[categoryName].map((interest: any) => (
                        <li key={interest.id} className="bg-[#1a1a1a] p-4 border-l-2" style={{ borderLeftColor: partyColour }}>
                          <p className="text-[13px] text-[#999999] whitespace-pre-wrap leading-[1.7] mb-2">{interest.interest_text}</p>
                          {interest.child_interests && interest.child_interests.length > 0 && (
                            <ul className="mt-2 ml-3 space-y-1">
                              {interest.child_interests.map((child: any, idx: number) => (
                                <li
                                  key={idx}
                                  className="text-[12px] text-[#999999] bg-[#111827] p-2 border-l-2 leading-[1.7]"
                          ...
                          <p className="text-[11px] text-[#999999] mt-2 font-mono">
                ...
                <div>
                  ...
                  <ul className="space-y-px bg-[#2a2a2a] border border-[#2a2a2a]">
                    {committeeMemberships.map((committee: any, idx: number) => (
                      <li key={idx} className="bg-[#1a1a1a] p-4 border-l-2" style={{ borderLeftColor: partyColour }}>
...
function Card({ title, children, partyColour }: { title: string; children: React.ReactNode; partyColour: string }) {
  return (
    <div className="bg-[#1a1a1a] p-5 border-l-2" style={{ borderLeftColor: partyColour }}>
...
function Empty({ children }: { children: React.ReactNode }) {
  return <p className="text-[#999999] text-[13px] leading-[1.7]">{children}</p>
}

function RolesRow({ post, partyColour }: { post: any; partyColour: string }) {
  return (
    <li className="bg-[#1a1a1a] p-4 border-l-2" style={{ borderLeftColor: partyColour }}>
      <p className="text-[13px] text-white font-semibold leading-snug">{post.name}</p>
      <p className="text-[11px] text-[#999999] font-mono mt-1">
