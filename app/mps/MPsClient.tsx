  ...
  return (
    <main className="max-w-7xl mx-auto px-4 sm:px-6 py-10 sm:py-16">
      <header className="border-b border-[#1c3849] pb-10 mb-10">
        <p className="text-[10px] uppercase tracking-[0.3em] text-[#9bdd42] font-medium mb-4">
      ...
      <div className="mb-12">
        ...
        <input
          ...
          className="w-full bg-[#1c3849] text-white text-sm leading-[1.7] border border-[#1c3849] rounded-sm px-4 py-3 placeholder:text-[#7697a2] focus:outline-none focus:border-[#9bdd42] transition-colors"
        />
      </div>
      ...
      {filtered.length === 0 ? (
        ...
        <div className="space-y-16">
          {parties.map((party) => {
            const partyColour = byParty[party][0]?.party_colour || '#9bdd42'
              ...
              <section key={party} className="border-t border-[#1c3849] pt-8">
                ...
                <ul className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-px bg-[#1c3849] border border-[#1c3849]">
                  {byParty[party].map((mp) => (
                    <li key={mp.id} className="bg-[#002633]">
                      <Link
                        href={`/mps/${mp.member_id}`}
                        className="group flex items-start gap-3 bg-[#1c3849] hover:bg-[#002633] transition-colors p-4 border-l-2 border-transparent hover:border-l-[#9bdd42]"
                      >
                        {mp.photo_url ? (
                          <img
                            src={mp.photo_url}
                            alt={mp.name}
                            className="w-12 h-12 rounded-full object-cover bg-[#002633] flex-shrink-0"
                            style={{ border: `1px solid ${partyColour}` }}
                          />
                        ) : (
                          <div
                            className="w-12 h-12 rounded-full bg-[#002633] flex items-center justify-center text-[#7697a2] text-[10px] uppercase tracking-wider flex-shrink-0"
                            style={{ border: `1px solid ${partyColour}` }}
                          >
                            {mp.name?.charAt(0)}
                          </div>
                        ...
                        <div className="flex-1 min-w-0">
                          <h3 className="font-semibold text-white text-[14px] leading-snug truncate group-hover:text-[#9bdd42] transition-colors">
                            {mp.name}
                          </h3>
