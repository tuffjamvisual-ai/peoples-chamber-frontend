    ...
    <div className="w-full">
      ...
      <div className="bg-[#001520] border border-[#1c3849] rounded-lg p-4">
        <div className="flex items-center justify-between mb-3">
          ...
          {poll.constituency && (
            <span className="text-xs px-2 py-0.5 bg-[#1c3849] text-[#7697a2] rounded border border-[#405b6b]">{poll.constituency}</span>
        )}
        </div>
        ...
        <div className="flex gap-2">
          ...
          <button
            ...
            disabled={hasVoted}
            className={'px-4 py-1.5 rounded text-xs font-medium transition-colors ' + (hasVoted ? (userVote === 'no' ? 'bg-[#8a3a3a] text-white' : 'bg-[#405b6b] text-[#7697a2] cursor-not-allowed') : 'bg-[#8a3a3a] hover:bg-[#8a3a3a] text-white')}
          >
            {userVote === 'no' ? '✓ No' : 'No'}
          </button>
        </div>
