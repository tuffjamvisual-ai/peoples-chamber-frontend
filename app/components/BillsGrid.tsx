...
export default function BillsGrid({ initialBills, hideFilters = false }: Props) {
  ...
  const filteredBills = useMemo(() => {
    ...
    if (categoryFilter) {
      filtered = filtered.filter((bill: any) => bill.category === categoryFilter);
    }

    if (houseFilter) {
      filtered = filtered.filter((bill: any) => bill.originating_house === houseFilter);
    }

    if (sessionFilter) {
      filtered = filtered.filter((bill: any) => bill.introduced_session_id?.toString() === sessionFilter);
    }

    if (stageFilter) {
      filtered = filtered.filter((bill: any) => bill.current_stage === stageFilter);
    }

    if (parliamentVotedFilter) {
      filtered = filtered.filter((bill: any) => bill.commons_ayes !== null && bill.commons_noes !== null);
    }
    ...
    if (hasSummaryFilter) {
      filtered = filtered.filter((bill: any) => bill.plain_summary);
    }
    ...
    return filtered;
  }, [bills, searchTerm, categoryFilter, houseFilter, sessionFilter, stageFilter, 
      sortBy, parliamentVotedFilter, youVotedFilter, notVotedFilter, hasSummaryFilter, userVotes]);
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [searchTerm, categoryFilter, houseFilter, sessionFilter, stageFilter,
      parliamentVotedFilter, youVotedFilter, notVotedFilter, hasSummaryFilter]);
    ...
    <>
      <div className="mb-6">
        <input
          ...
          className="max-w-2xl w-full px-4 py-3 bg-gray-800 text-white rounded-lg border border-gray-700 focus:border-blue-500 focus:outline-none"
        />
      </div>
      ...
      {paginatedBills.length > 0 && totalPages > 1 && (
        <div className="flex items-center justify-center gap-2">
          <button
            ...
            className="px-3 py-1.5 bg-gray-800 text-gray-300 rounded text-sm hover:bg-gray-700 disabled:opacity-30"
          >
            First
          </button>
          <button
