// Sync ministers' meetings — pulls publication-level metadata from GOV.UK
// Search API for transparency-data documents tagged "ministerial meetings".
// Stored per-publication; per-meeting CSV parsing is a separate concern.
...
const QUERY = 'ministerial meetings';
const PAGE_SIZE = 100;
...
async function main() {
  console.log('[ministers-meetings] fetching from GOV.UK search API…');
  const rows = await fetchAll();
  console.log(`[ministers-meetings] fetched ${rows.length} publications`);
  if (rows.length === 0) { console.log('Nothing to upsert.'); return; }
  const { error } = await supabase
    .from('ministers_meetings')
    .upsert(rows, { onConflict: 'link' });
  if (error) {
    console.error('[ministers-meetings] upsert error:', error.message || error);
  } else {
    console.log(`[ministers-meetings] upsert ok (${rows.length} rows)`);
  }
}
