...
async function calculateRebellions() {
  ...
  const uniqueDivisions = [...new Set(allVotes?.map(d => d.division_id).filter(id => id))];
  ...
  for (const divisionId of uniqueDivisions) {
    try {
      ...
      console.log(`  Division ${divisionId}: ${votes?.length || 0} votes to process`);
      ...
      processedDivisions++;
      console.log(`[${processedDivisions}/${uniqueDivisions.length}] Completed - ${rebellionsFound} total rebellions found\n`);
